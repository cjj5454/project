...
const pageId = 'CH010102_demoEtidVO';
        ...
        (data) => {
            if (data) {
                ...
                if (data.button) {
                    ...
                    props.button.setPopContent('DelLine', this.state.json['CH010102-000019']) /*国际化：确认要删除该信息吗？ 设置操作列上删除按钮的弹窗提示 */
                }
            }
            callback();
        }
...
export function onClickButton(props, id) {
    ...
    switch (id) {
        case 'Add':
            ...
            props.button.setMainButton('Test2', false);
            break;
        case 'Cancel':
            promptBox({
                color: "warning",
                title: this.state.json['CH010102-000006'],/* 国际化处理： 确认取消*/
                content: this.state.json['CH010102-000007'],/* 国际化处理： 是否确认要取消!*/
                beSureBtnClick: () => {
                ...
                ajax({
                    ...
                    success: function (res) {     //此处使用箭头函数，如果不使用箭头函数，一定要bind(this)
                        ...
                        if (res.data) {
                            ...
                            toast({ color: 'success', title: this.state.json['CH010102-000008'] });/* 国际化处理： 保存成功*/
                        }
                        props.button.setButtonDisabled('Edit', true);
                        props.button.setMainButton('Add', true);
                        props.button.setButtonDisabled('Delete', true);
        ...
        case 'Delete':
            if (rowsdata.length != 0) {
                if (props.editTable.getStatus(gridid) && props.editTable.getStatus(gridid) == 'edit') {
                    ...
                    promptBox({
                        color: "warning",
                        title: this.state.json['CH010102-000009'],/* 国际化处理： 确认删除*/
                        content: this.state.json['CH010102-000010'],/* 国际化处理： 确定要删除所选数据吗？*/
                ...
                promptBox({
                    color: "warning",
                    title: this.state.json['CH010102-000011'],/* 国际化处理： 提示*/
                    content: this.state.json['CH010102-000012']/* 国际化处理： 请选择数据操作!*/
                });
            }
            break;
        case 'PrintGrp':
        ...
        case 'Output':
            ...
            break;
        case 'Test2':
            setvalue.call(this, props);
            alert(22);
            break;
        case 'Refresh':
            loadGridData.call(this, false, paramData);
            toast({ title: this.state.json['CH010102-000020'], color: 'success' });/* 国际化处理： 无可打印数据*/
        default:
            break;
    }
}
 ...
 * @param {*} props 
 */
export function setvalue(props){
    let rowall=props.editTable.getAllRows(gridid);
    for(let onerow of rowall){

        props.editTable.setValByKeyAndRowId(gridid,onerow.rowid,'demo_depa',{value:'1',display:'2'})
        onerow.rowid
    }
}
/**
...
export function gridStatusChange() {
    ...
    this.props.button.setPopContent('DelLine', gridStatus === 'browse' ? this.state.json['CH010102-000000'] : '');/* 国际化处理： 确定要删除吗？*/
}
...
export function onClickSearchBtn(props, data) {
    if (props.editTable.getStatus() && props.editTable.getStatus() == 'edit') {
        promptBox({
            color: "warning",
            title: this.state.json['CH010102-000011'],/* 国际化处理： 提示*/
            content: this.state.json['CH010102-000015']/* 国际化处理： 编辑态不能查询*/
        });
    };
    let OID = this.props.meta.getMeta()[searchid].oid;
    let paramdata = {
    ...
    ajax({
        ...
        success: (res) => {
            if (res.success) {
                ...
                loadGridData.call(this, false, paramData);
                toast({ color: 'success', title: this.state.json['CH010102-000002'] });/* 国际化处理： 删除成功*/
            } else {
                alert(res.message);
            }
        }
...
function onPrint() {
    ...
    if (allData.length === 0) {
        toast({ content: this.state.json['CH010102-000013'], color: 'warning' });/* 国际化处理： 无可打印数据*/
        return;
    }
    let pks = [];
...
function onOutput() {
    let allData = this.props.editTable.getAllData(gridid);
    if (allData.length === 0) {
        toast({ content: this.state.json['CH010102-000014'], color: 'warning' });/* 国际化处理： 无可输出的数据*/
        return;
    }
    let pks = [];
    allData.rows.forEach((item, key) => {
    ...
    ajax({
        ...
        success: (res) => {
            ...
            if (success) {
                ...
                if (isToast === true) {
                    toast({ color: 'success', content: this.state.json['CH010102-000003'] + data[gridid].allpks.length + this.state.json['CH010102-000004'] });/* 国际化处理： 查询成功，共,条。*/
                }
        ...
        error: (res) => {
            if (isToast === true) {
                toast({ color: 'warning', content: this.state.json['CH010102-000005'] });/* 国际化处理： 未查询出符合条件的数据*/
            }
        }
    })
...
function modifierMeta(props, meta) {
    ...
    meta[gridid].items.push({
        attrcode: 'opr',
        label: this.state.json['CH010102-000001'],/* 国际化处理： 操作*/
        width: 200,
        fixed: 'right',
        className: 'table-opr',
        itemtype: 'customer',
